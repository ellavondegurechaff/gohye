    ...
    bonus: async (ctx, user, item, index, args) => {
        switch (item.id) {
            case 'slotupgrade':
                ...
                const expiry = asdate.add(new Date(), 210, 'days')
            ...
            case 'effectincrease':
                ...
                if (chosenEffect.expires)
                    update = {expires: asdate.add(chosenEffect.expires, 3, 'days')}
                else
                    update = {uses: chosenEffect.uses + 1}

                await UserEffect.findOneAndUpdate({userid: user.discord_id, id: chosenEffect.id}, update)

                await ctx.reply(user, `you have successfully increased the ${chosenEffect.expires? 'expiration time': 'use count'} for **${itemEffect.name}** by 3 ${chosenEffect.expires? 'day': 'use'}!`, 'green', true)
                break
            case 'legendswapper':
    ...
    recipe: (ctx, user, item) => {
        const cards = item.recipe.reduce((arr, x) => {
            arr.push(_.sample(ctx.cards.filter(y => y.level === x 
                ...
                && !ctx.collections.find(z => z.id === y.col).fragments
                && !ctx.collections.find(z => z.id === y.col).lottery
                && !ctx.collections.find(z => z.id === y.col).jackpot
                && !ctx.collections.find(z => z.id === y.col).album
