...
const { 
    countUserCards, getUserCards, removeUserCards, addUserCards
} = require('./user')
...
const reset = async (ctx, user, col, amounts) => {
    ...
    const cardsToRemove = userCards.filter(x => {
        ...
        if (exists) {
            ...
            const notLegendary = (isFragmentCollection && level === 1) || (!isFragmentCollection && level < 5);
    ...
    if (legendary) {
        if(isFragmentCollection){
            // Add the 4-star card to the user's inventory
            await addUserCards(ctx, user, [legendary.id]);
            await user.save

        } else {
            const legendaryTicket = new userInventory({
            ...
            await legendaryTicket.save();
            // user.inventory.push(legendaryTicket); // You can still push to the user object if you need it there
        }
    }

    totalEvalPrice = await calculateCollectionEval(ctx, col.id);
    ...
    // Update the user's flake balance
    user.exp += flakesToReturn;

    await user.save();

    if(isFragmentCollection){
        return ctx.reply(user, `You successfully reset **${col.name}**! The 4 ⭐ card **${legendary.name.toUpperCase()}** has been added to your card inventory`, 'green', true);
    } else {
        return ctx.reply(user, `You successfully reset **${col.name}**! ${legendary? `A legendary card ticket was added to your inventory. Use it to draw **${col.id} legendary card**!` : ''}`, 'green', true);
    }
}
